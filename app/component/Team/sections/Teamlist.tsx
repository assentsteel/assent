      ...
      <div className="container">
        ...
        >
          <motion.div
            ...
            onClick={() => setSelectedMember(data.teamSection.items[0])}
          >
            <div
              className={`group rounded-[15px] overflow-hidden flex flex-col w-full ${
                data.teamSection.items[0].description ? "cursor-pointer" : ""
              }`}
            >
              <div className="bg-[#CACBCA] group-hover:bg-primary transition-all duration-500">
                <figure className="">
                  <Image
                    ...
                    width={500}
                    height={500}
                    className="grayscale-[1] group-hover:grayscale-0 transition-all duration-400 w-full object-cover object-top pointer-events-none"
                  />
                </figure>
              </div>
              <div className="px-5 py-5 lg:px-10 lg:py-5 bg-[#F5F5F5] rounded-b-[15px] flex-1">
                <p className="text-md font-semibold text-territory">
                  {data.teamSection.items[0].name}
                </p>
                <p className="text-[#595959]">
                  {data.teamSection.items[0].designation}
                </p>
              </div>
            </div>
          </motion.div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          {data.teamSection.items
            .slice(1, data.teamSection.items.length)
            .map((item, index) => (
              <motion.div
                key={index}
                variants={itemVariants}
                onClick={() => setSelectedMember(item)}
                className="w-full md:w-[calc(50%-16px)] lg:w-[calc(33.333%-22px)] xxl:w-[calc(25%-24px)] flex"
              >
                <div
                  className={`group rounded-[15px] overflow-hidden flex flex-col w-full ${
                        ...
                        {selectedMember.linkedIn &&
                          selectedMember.linkedIn !== "" && (
                            <Image
                              ...
                              className="cursor-pointer lg:mt-2 pointer-events-none"
                            />
                          )}
                      </div>
                      <p className="lg:text-sm text-xs text-gray-500 lg:mb-2 text-center lg:text-start">
