...
const Commitments = ({ data }: { data: Hse }) => {
  ...
  const handlePrev = () => {
    setCurrentIndex(
      (prev) =>
        (prev - 1 + data.secondSection.items.length) %
        data.secondSection.items.length
    );
  };
              ...
              <div className="mb-8 lg:mb-0">
                ...
                <motion.div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: data.secondSection.items[currentIndex].description,
                  }}
                ></motion.div>
                ...
                <div className="flex flex-col md:flex-row lg:flex-col xl:flex-row justify-center items-center md:justify-start gap-5  ">
                  {data.secondSection.items[currentIndex].files.map(
                    (file, index) => (
                      <button
                        key={index}
                        className="mb-4 md:mb:0  group flex justify-between items-center mt-auto border border-secondary py-2 px-6 rounded-full hover:bg-white hover:text-territory text-white transition text-xs h-[40px] lg:h-[48px] max-w-[280px] w-[280px] font-medium uppercase relative"
                      >
                        ...
                        <div className="absolute right-5 flex gap-2 items-center opacity-0 translate-y-1 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 ease-in-out">
                          ...
                          <Image
                            ...
                            width={22}
                            height={14}
                            onClick={() => setOpenCertificateId(file._id)}
                          />
                        </div>
                      </button>
                    )
          ...
          >
            ...
            <div className="w-full h-full mt-10">
              ...
              >
                <Viewer
                  fileUrl={
                    data.secondSection.items[currentIndex].files.find(
                      (file) => file._id === openCertificateId
                    )?.file || ""
                  }
                />
              </Worker>
