...
const [filteredResults, setFilteredResults] = useState<ProjectItem[]>(data); 
...
const handleCategoryChange = (value: { value: string; label: string }) => {
  const updated = [
    selectedValues[0],
    selectedValues[1],
    value,
...
const applyFilters = (filters: { value: string; label: string }[]) => {
  console.log(filters)
  const [country, sector,category,search] = filters;
  ...
  const results = filtered.filter((item) => {
    ...
    return matchCountry && matchSector && matchCategory && matchSearch;
  });
            ...
            <div className="lg:flex gap-20 mb-5 lg:mb-10 pb-5 lg:pb-10 border-b border-[#00000015]">
              <div className="md:flex w-full items-center uppercase text-md font-[500] gap-3 lg:gap-10 mb-5 lg:mb-0">
                <p>Filter</p>
                  ...
                  <div className="relative w-full my-3 lg:my-0">
                    <Listbox
                      value={selectedValues[2].value.split(" ").map((item) => item == "Projects" ? "" : item).join(" ")}
                      onChange={(val) => handleCategoryChange({value:val,label:val})}
                    >
                      <div className="relative">
                        <Listbox.Button className="uppercase w-full text-left text-xs text-primary bg-transparent border-0 border-b border-black py-2 pr-6 appearance-none focus:outline-none focus:ring-0 focus:border-black relative">
                          {selectedValues[2].value.split(" ").map((item) => item == "Projects" ? "" : item).join(" ")}
                          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-secondary pointer-events-none">
                            <ChevronUpDownIcon className="h-4 w-4" />
                          </span>
                        </Listbox.Button>
                        <Listbox.Options className="absolute z-10 left-0 w-full bg-white border mt-1 shadow-lg rounded-md max-md:h-[200px] overflow-auto">
                          {categoryData.map((option) => (
                            ...
                            >
                              {({ selected }) => (
                                <span className="flex items-center justify-between">
                                  {option.name.split(" ").map((item) => item == "Projects" ? "" : item).join(" ")}
                                  {selected && <CheckIcon className="w-4 h-4 ml-2 text-white" />}
                                </span>
                              )}
                            </Listbox.Option>
...
> 
        ...
        <Link href={`/projects/${item.categorySlug}/${item.slug}`}> 
