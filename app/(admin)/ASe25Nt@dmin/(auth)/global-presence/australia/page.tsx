...
export interface GlobalPresenceAustralia {
  seo: SeoFormValues;
...
const GlobalPresenceAustraliaPage = () => {
  const {
    register,
    handleSubmit,
    setValue,
    ...
    formState: { errors },
    // watch,
  } = useForm<GlobalPresenceAustralia>();
  ...
  const handleAddGlobalPresenceAustralia = async (data: GlobalPresenceAustralia) => {
    try {
      const response = await fetch(`/api/admin/global-presence/australia`, {
        method: "PATCH",
      ...
      console.log("Error in adding global presence australia", error);
    }
  };

  const fetchGlobalPresenceAustraliaData = async () => {
    try {
      const response = await fetch(`/api/admin/global-presence/australia`);
      if (response.ok) {
        const data = await response.json();
  ...
  useEffect(() => {
    fetchGlobalPresenceAustraliaData();
  }, []);
  ...
  return (
    <div className="flex flex-col gap-5">
      <form
        ...
        onSubmit={handleSubmit(handleAddGlobalPresenceAustralia)}
      >
        <AdminItemContainer>
        ...
        <SeoFields<GlobalPresenceAustralia> control={control} register={register} errors={errors} />
...
export default GlobalPresenceAustraliaPage;
