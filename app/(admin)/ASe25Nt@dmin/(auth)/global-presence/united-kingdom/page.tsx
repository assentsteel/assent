...
export interface GlobalPresenceUnitedKingdom {
  seo: SeoFormValues;
...
const GlobalPresenceUnitedKingdomPage = () => {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const handleAddGlobalPresenceUnitedKingdom = async (data: GlobalPresenceUnitedKingdom) => {
    try {
      const response = await fetch(`/api/admin/global-presence/united-kingdom`, {
        method: "PATCH",
      ...
      console.log("Error in adding global presence united kingdom", error);
    }
  };

  const fetchGlobalPresenceUnitedKingdomData = async () => {
    try {
      const response = await fetch(`/api/admin/global-presence/united-kingdom`);
      if (response.ok) {
        const data = await response.json();
  ...
  useEffect(() => {
    fetchGlobalPresenceUnitedKingdomData();
  }, []);
  ...
  return (
    <div className="flex flex-col gap-5">
      <form
        ...
        onSubmit={handleSubmit(handleAddGlobalPresenceUnitedKingdom)}
      >
        <AdminItemContainer>
        ...
        <SeoFields<GlobalPresenceUnitedKingdom> control={control} register={register} errors={errors} />
...
export default GlobalPresenceUnitedKingdomPage;
