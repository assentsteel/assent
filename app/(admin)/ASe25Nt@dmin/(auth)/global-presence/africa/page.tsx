...
export interface GlobalPresenceAfrica {
  seo: SeoFormValues;
...
const GlobalPresenceAfricaPage = () => {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const handleAddGlobalPresenceAfrica = async (data: GlobalPresenceAfrica) => {
    try {
      const response = await fetch(`/api/admin/global-presence/africa`, {
        method: "PATCH",
      ...
      console.log("Error in adding global presence africa", error);
    }
  };

  const fetchGlobalPresenceAfricaData = async () => {
    try {
      const response = await fetch(`/api/admin/global-presence/africa`);
      if (response.ok) {
        const data = await response.json();
  ...
  return (
    <div className="flex flex-col gap-5">
      <form
        ...
        onSubmit={handleSubmit(handleAddGlobalPresenceAfrica)}
      >
        ...
        <SeoFields<GlobalPresenceAfrica> control={control} register={register} errors={errors} />
...
export default GlobalPresenceAfricaPage;
