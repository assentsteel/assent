...
export interface GlobalPresenceUsa {
  seo: SeoFormValues;
...
const GlobalPresenceUsaPage = () => {
  ...
  const handleAddGlobalPresenceUsa = async (data: GlobalPresenceUsa) => {
    try {
      const response = await fetch(`/api/admin/global-presence/usa`, {
        method: "PATCH",
      ...
      console.log("Error in adding global presence usa", error);
    }
  };

  const fetchGlobalPresenceUsaData = async () => {
    try {
      const response = await fetch(`/api/admin/global-presence/usa`);
      if (response.ok) {
        const data = await response.json();
  ...
  return (
    <div className="flex flex-col gap-5">
      <form
        ...
        onSubmit={handleSubmit(handleAddGlobalPresenceUsa)}
      >
        <AdminItemContainer>
        ...
        <SeoFields<GlobalPresenceUsa> control={control} register={register} errors={errors} />
...
export default GlobalPresenceUsaPage;
