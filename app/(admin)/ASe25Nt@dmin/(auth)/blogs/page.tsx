...
export default function Blogs() {
    ...
    const [blogsList, setBlogsList] = useState<{ _id: string; mainTitle: string; thumbnail: string }[]>([]);
    ...
    const handleFetchMeta = async () => {
        try {
            const response = await fetch("/api/admin/blogs/intrometa");
            if (response.ok) {
                const data = await response.json();
        ...
        <div className="h-fit grid grid-cols-1 gap-5">
            ...
            <div className="h-[400px] w-full p-2 border-2 border-gray-300 rounded-md overflow-y-hidden">
                {/* <div className="flex justify-between border-b-2 pb-2">
                    ...
                    <Button onClick={() => router.push("/ASe25Nt@dmin/blogs/add")} className="text-white text-[16px]">
                        Add Blogs
                    </Button>
                </div> */}
                <div className="flex justify-between items-center border-b-2 pb-2 gap-4">
                    ...
                    <Button
                        onClick={() => router.push("/ASe25Nt@dmin/blogs/add")}
                ...
                <div className="mt-2 flex flex-col gap-2 overflow-y-scroll h-3/4">
                    {filteredBlogs?.map((item) => (
                        ...
                        >
                            ...
                            <div className="flex gap-5">
                                <MdEdit onClick={() => router.push(`/ASe25Nt@dmin/blogs/edit/${item._id}`)} />
