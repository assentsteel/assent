...
<AdminItemContainer>
                ...
                <Button disabled={fields.length < 1} type="button" className={`text-white text-[16px] ${reorderMode ? "bg-yellow-700" : "bg-green-700"}`} onClick={() => setReorderMode(!reorderMode)}>{reorderMode ? "Done" : "Reorder"}</Button>
            ...
            <DndContext collisionDetection={closestCorners} onDragEnd={handleDragEnd}>
                <SortableContext items={fields.map((item) => item.id)} strategy={verticalListSortingStrategy}>
                    {fields?.map((item, index) => (
                        <MemberCard key={index} item={item} id={item.id} />
                    ))}
                </SortableContext>
            </DndContext>
